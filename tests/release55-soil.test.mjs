...
test('V55 exposes soil tools on desktop and mobile entry paths',()=>{
  ...
  assert.equal(pkg.version,'0.55.0');assert.match(html,/AMBIENTE TEST · V55/);assert.match(html,/src\/app\.js\?v=55/);assert.match(html,/id="soil-button"/);assert.match(html,/id="soil-analyze"/);assert.match(mobile,/#soil-button/);
});
