...
test('map renders per-side measurement labels from the editable polygon', () => {
  assert.match(mapSource, /sideMeasurements/);
  assert.match(mapSource, /sideMeasurementsToFeatureCollection/);
  assert.match(mapSource, /type:\s*'symbol'/);
