-- A project saved before versioned history was introduced can still have a
-- valid field_plans snapshot and a public code without a project_revisions row.
-- Keep the same rate limit and saved-only access rule; do not grant table reads.
...
begin
  ...
  from jsonb_array_elements(selected_fields) with ordinality selected(field,ordinal)
  where jsonb_typeof(field->'geometry')='array'
    and jsonb_array_length(field->'geometry')>=4;
  if jsonb_array_length(public_fields)=0 then
    insert into private.project_public_lookup_attempts(requester_hash,successful)
    values(requester_hash_value,false);
    return null;
  end if;
